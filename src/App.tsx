...
function cleanGeneratedText(text: string): string {
  ...
  if (cleaned.match(/前情(?:解码|提要|回顾)/) && cleaned.match(/(?:大结局)?正文/)) {
    cleaned = cleaned.replace(/前情(?:解码|提要|回顾)[\s\S]*?(?:大结局)?正文[：:]?/g, '');
  } else {
  ...
  // 3. 兜底删除残留的过渡词
  cleaned = cleaned.replace(/(?:大结局)?正文[：:]?/g, '');
  cleaned = cleaned.replace(/前情(?:解码|提要|回顾)[：:]?/g, '');
