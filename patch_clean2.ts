...
const regexFunc = /function cleanGeneratedText\(text: string\): string \{[\s\S]*?return cleaned\.trim\(\);\n\}/m;
...
// Remove the generated text preview from card
const regexPreview = /\{\/\* Generated Text View \(always show if we have results, so we can see intermediate steps\) \*\/\}\s*\{task\.subResults\.length > 0 && \([\s\S]*?<\div>\s*\)\}/m;
